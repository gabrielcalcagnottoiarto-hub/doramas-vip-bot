...
const VIP_PRICE_USD = process.env["VIP_PRICE_USD"] ?? "7.00";
const WISE_EMAIL = process.env["WISE_EMAIL"] ?? "";
  ...
  bot.on("callback_query", async (query) => {
    ...
    if (data === "yuna_profile") {
      try {
        await bot.sendPhoto(chatId, YUNA_PHOTO, {
          caption:
            "🌸 *Yuna — Sua Narradora IA Sensual*\n\n" +
            "Eu sou a Yuna. Fui criada para sussurrar no seu ouvido as histórias que ninguém mais vai contar.\n\n" +
            "🤖 *Minha Tecnologia:*\n" +
            "🖼️ Rosto e corpo gerados por IA (Pollinations AI / Flux)\n" +
            "🎬 Vídeos animados com lip-sync e movimento real (D-ID / Replicate)\n" +
            "🗣️ Voz neural hiper-realista em 10 idiomas (Microsoft Neural TTS)\n" +
            "🎵 Trilha sonora ambiente gerada por IA\n" +
            "📸 Modo Realista com qualidade Unreal Engine 5\n\n" +
            "💜 Sou 100% inteligência artificial — cada imagem, cada voz, cada vídeo é gerado em tempo real, exclusivamente para você.\n\n" +
            "✦ Sua narradora pessoal. Sempre aqui. Sempre pronta. ✦",
          parse_mode: "Markdown",
    ...
    if (data === "assinar_vip") {
      ...
      if (OPENPIX_API_KEY) {
        ...
        if (result.ok) {
          const { charge } = result;
          const tonNanoAuto = Math.round(Number(VIP_PRICE_TON.replace(",", ".")) * 1_000_000_000);
          const tonkeeperUrlAuto = TONCOIN_ADDRESS ? `https://app.tonkeeper.com/transfer/${TONCOIN_ADDRESS}?amount=${tonNanoAuto}&text=VIP_${userId}` : "";
          const pixTxt =
            `👑 *VIP DoramaAI — Pagamento*\n\n` +
            `✦ O que você ganha:\n🎥 150 episódios em *HD*\n🚫 Zero propaganda\n⚡ *Ativação automática* ao pagar!\n\n` +
            `💰 *R$ ${VIP_PRICE_BRL}* · *${VIP_PRICE_TON} TON* · *$${VIP_PRICE_USD} USD*\n\n` +
            `📲 *PIX Copia e Cola:*\n\`${charge.brCode.slice(0, 80)}...\`\n\n` +
            `⏱️ QR Code válido por 1 hora\n✅ VIP ativado *automaticamente* após pagamento!`;

          const autoButtons: { text: string; url?: string; callback_data?: string }[][] = [
            [{ text: "📋 Copiar código PIX", callback_data: `pix_copy_${charge.correlationID}` }],
            [{ text: "🔗 Link de pagamento", url: charge.paymentLinkUrl }],
          ];
          if (tonkeeperUrlAuto) {
            autoButtons.push([{ text: `💎 Pagar ${VIP_PRICE_TON} TON via Tonkeeper`, url: tonkeeperUrlAuto }]);
          }
          if (WISE_EMAIL) {
            const wiseUrlAuto = `https://wise.com/pay#email=${encodeURIComponent(WISE_EMAIL)}&amount=${VIP_PRICE_USD}&currency=USD&description=VIP_${userId}`;
            autoButtons.push([{ text: `💸 Pagar $${VIP_PRICE_USD} via Wise`, url: wiseUrlAuto }]);
          }
          autoButtons.push([{ text: "🏠 Menu Principal", callback_data: "menu" }]);
          ...
          try {
            if (charge.qrCodeImage) {
              await bot.sendPhoto(chatId, charge.qrCodeImage, {
                caption: pixTxt,
                parse_mode: "Markdown",
                reply_markup: { inline_keyboard: autoButtons },
              });
            } else {
      ...
      txt += `💰 *R$ ${VIP_PRICE_BRL}* · *${VIP_PRICE_TON} TON* · *$${VIP_PRICE_USD} USD*\n\n`;
      if (PIX_KEY) txt += `📲 *PIX:* \`${PIX_KEY}\`\n`;
      if (TONCOIN_ADDRESS) txt += `💎 *TON:* \`${TONCOIN_ADDRESS}\`\n`;
      if (WISE_EMAIL) txt += `💸 *Wise:* \`${WISE_EMAIL}\` ($${VIP_PRICE_USD})\n`;
      txt += `\n📩 Envie o comprovante aqui (foto ou texto).\n✅ VIP ativado em até 30 min!`;

      const vipButtons: { text: string; url?: string; callback_data?: string }[][] = [];
      if (TONCOIN_ADDRESS) {
        const tonNano = Math.round(Number(VIP_PRICE_TON.replace(",", ".")) * 1_000_000_000);
        const tonkeeperUrl = `https://app.tonkeeper.com/transfer/${TONCOIN_ADDRESS}?amount=${tonNano}&text=VIP_${userId}`;
        vipButtons.push([{ text: `💎 Pagar ${VIP_PRICE_TON} TON via Tonkeeper`, url: tonkeeperUrl }]);
      }
      if (WISE_EMAIL) {
        const wiseUrl = `https://wise.com/pay#email=${encodeURIComponent(WISE_EMAIL)}&amount=${VIP_PRICE_USD}&currency=USD&description=VIP_${userId}`;
        vipButtons.push([{ text: `💸 Pagar $${VIP_PRICE_USD} via Wise`, url: wiseUrl }]);
      }
      if (PIX_KEY) {
        vipButtons.push([{ text: "📲 Copiar chave PIX", callback_data: "pix_copy_manual" }]);
      }
      vipButtons.push([{ text: "🏠 Menu Principal", callback_data: "menu" }]);

      const vipReply = { inline_keyboard: vipButtons };
      if (msgId) {
        try {
          await bot.editMessageText(txt, { chat_id: chatId, message_id: msgId, parse_mode: "Markdown", reply_markup: vipReply });
        } catch {
          await bot.sendMessage(chatId, txt, { parse_mode: "Markdown", reply_markup: vipReply });
        }
      } else {
        await bot.sendMessage(chatId, txt, { parse_mode: "Markdown", reply_markup: vipReply });
      }
