...
async function animateImageWithReplicate(
  ...
  try {
    ...
    // Style-aware animation prompt — maximum visible movement
    const animationPrompt = imageStyle === "realistic"
      ? "continuous body movement, sensual swaying hips, chest rising and falling with deep breathing, hair blowing in wind, head turning slowly side to side, eyes blinking naturally, lips parting slightly, shoulders rolling, fingers moving gently, fabric flowing and rippling, cinematic slow motion camera pan, shallow depth of field with bokeh, photorealistic fluid motion, skin glistening, dramatic lighting shifts"
      : "dynamic anime movement, long hair flowing and swirling dramatically in wind, visible breathing with chest movement, expressive eye sparkles and blinks, clothes and fabric fluttering, cherry blossom petals swirling around, slow sensual body sway, dramatic camera orbit, glowing particle effects, cinematic anime motion, vibrant color shifts";
...
async function composeWithKenBurns(
  ...
  try {
    ...
    const args = [
      ...
      // Input: ambient music
      "-i", ambientPath,
      // Filters — Dynamic Ken Burns with zoom + pan for continuous visible movement
      "-filter_complex",
      [
        // Ken Burns: zoom oscillates between 1.0x-1.35x AND pans across image for constant motion
        // zoom: uses sine wave to zoom in/out cyclically (period ~10s at 24fps = 240 frames)
        // x/y: drift diagonally across the image using slow sine panning
        `[0:v]scale=1280:-2,zoompan=z='1.0+0.35*abs(sin(on/240*PI))':d=3120:x='(iw-iw/zoom)/2+30*sin(on/180*PI)':y='(ih-ih/zoom)/2+20*cos(on/220*PI)':s=720x720:fps=24[vid]`,
...
async function composeNoNarrationKenBurns(
  ...
  try {
    ...
    const args = [
      ...
      "-i", ambientPath,
      "-filter_complex",
      `[0:v]scale=1280:-2,zoompan=z='1.0+0.35*abs(sin(on/240*PI))':d=3120:x='(iw-iw/zoom)/2+30*sin(on/180*PI)':y='(ih-ih/zoom)/2+20*cos(on/220*PI)':s=720x720:fps=24[vid];[1:a]volume=0.6,afade=t=in:d=2,afade=t=out:st=${duration - 3}:d=3[audio]`,
      "-map", "[vid]",
      "-map", "[audio]",
      "-c:v", "libx264",
      "-preset", quality === "hd" ? "medium" : "fast",
